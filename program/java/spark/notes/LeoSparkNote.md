...
* When join using _round($"a",3)<=>round($"a",3)_, $"a" and $"b" should be of the same type (e.g Double/Float). When $"a" is of Double type and $"b" is of Float type, the join condition _round($"a",3)<=>round($"a",3)_ could be false even if $"a" and $"b" are equal.
